...
function IconEmail() {
  return (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden>
      ...
      <path d="M1.5 3.5L8 9l6.5-5.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

const SOCIALS = [
  { label: "GitHub", href: "https://github.com/anhht", icon: IconGitHub },
  { label: "LinkedIn", href: "https://linkedin.com/in/anhht", icon: IconLinkedIn },
  { label: "Email", href: "mailto:anhht.fe@gmail.com", icon: IconEmail },
];

export async function Sidebar() {
  const res = await serverFetch("/categories");
  const categories: CategoryData[] = res.ok ? await res.json() : [];
  const total = categories.reduce((n, c) => n + c.posts.length, 0);
