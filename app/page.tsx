  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-12">
      <header className="mb-12 border-b border-line pb-6">
        <h1 className="font-mono uppercase tracking-[2px] text-2xl">
          anhht blog
        </h1>
      </header>
      ...
      {categories.map((cat) => (
        <section key={cat.id} className="mb-16">
          <div className="flex items-center justify-between mb-4 border-b border-line pb-2">
            <h2 className="font-mono uppercase tracking-[2px] text-sm">
              {cat.name}
            </h2>
            <a
          ...
          {/* Bento grid — sẽ style chi tiết ở Phase 5 */}
          <div className="grid grid-cols-4 auto-rows-[128px] gap-3">
            {cat.posts.slice(0, 4).map((rel, idx) => {
              const post = rel.post;
              ...
              return (
                <a
                  ...
                  href={`/blog/${post.slug}`}
                  className={[
                    "border border-line bg-surface p-3 flex flex-col justify-between",
